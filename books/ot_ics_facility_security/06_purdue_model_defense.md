---
title: "第6章：Purdueモデルに基づいたDMZと多層防御"
---

# 第6章：Purdueモデルに基づいたDMZと多層防御
...
画面に `tcpdump: listening on any...` と表示されたら、ターミnal2から以下の攻撃コマンドを実行します。
...
## 4. なぜ Wireshark ではなく「自作IDSパーサー」を作るのか？
...
取得したpcapを分析し、正常系ベースラインからの「逸脱（アノマリー）」をプログラムで自動抽出するパーサー（`ids_parser.py`）を作成します。
...
本ラボを通して理解できるITとOTの決定的な違い、それは**「OT環境におけるサイバー攻撃の本質は、メモリの書き換えである」**ということです。
...
本書で構築した「OTセキュリティラボ」の完全なソースコードとDocker環境を、GitHubで公開しています。

[schutzz/ot-security-lab](https://github.com/schutzz/ot-security-lab)

`docker-compose up -d` を実行するだけで、Rust製エミュレータ・Node-RED監視基盤・検証スクリプト・自作IDS のすべてが手元で立ち上がります。ぜひご自身の手で体験してみてください。
